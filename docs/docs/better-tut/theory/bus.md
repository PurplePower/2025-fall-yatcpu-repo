...
AXI 协议是由 ARM 公司提出的总线通信协议，目前最新版本是 AXI4，并且提供了精简的 AXI4-Lite 协议。AXI 协议的主要特性如下：
...
AXI 协议只有两类接口：**主设备（Master）**和**从设备（Slave）**，它们的信号正好互为输入输出。主设备是指主动发送读写请求的一方，从设备则是完全被动地接受请求并完成。
...
AXI4 使用了 5 个 通道以完成上述地址、数据和响应信号传输，其中 AW、W 和 B 通道负责写操作，AR 和 R 负责读操作，如下图所示：

![axi-channels](images/axi-channels.png){width=80%}

每个通道都遵循**握手机制**以完成传输，**握手** 由 `VALID` 和 `READY` 信号完成。 `VALID` 表示通道上的信号有效，例如主设备将 `AWVALID` 置 1 时，说明此时 `AWADDR` 的值是有效的写地址。`READY` 信号表示准备好接受数据了，例如从设备将 `AWREADY` 置 1 时，说明其准备好接受写入地址。
...
/// caption
握手示例1：主设备将信息和 `VALID` 置 1 后，从设备再置 1 `READY` ，于第 4 周期完成握手。
///


![axi-hs-eg2](images/axi-handshake-eg2.png){width=90%}
/// caption
握手示例2：从设备提前置 1 `READY`，主设备将 `VALID` 置 1 后，于第 4 周期完成握手。
///
